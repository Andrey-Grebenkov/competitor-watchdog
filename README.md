...
- URL сайтов проходят `assertPublicUrl` (`src/lib/urlGuard.ts`): только http(s),
  ...
  `169.254.169.254`) и служебные домены. Скрапер проходит редиректы вручную
  (`maxRedirects: 0`), проверяя каждый переход, блокирует запросы подресурсов на
  внутренние адреса и не снимает скриншот, если итоговый URL страницы приватный.
