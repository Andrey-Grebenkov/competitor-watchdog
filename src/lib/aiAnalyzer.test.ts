...
describe("geminiEndpoint", () => {
  ...
  it("drops trailing slashes from the base URL", async () => {
    ...
    expect(geminiEndpoint("gemini-1.5-pro")).toBe(
      "https://gem.test/v1beta/models/gemini-1.5-pro:generateContent",
    );
  });

  it("strips a models/ prefix and expands gemini-1.5-flash", async () => {
    const { geminiEndpoint } = await loadAnalyzer({
      GEMINI_API_BASE: "https://gem.test/v1beta",
    });

    expect(geminiEndpoint("models/gemini-1.5-flash")).toBe(
      "https://gem.test/v1beta/models/gemini-1.5-flash-latest:generateContent",
    );
  });
});

describe("model chain", () => {
  it("puts the configured model first without duplicating fallbacks", async () => {
    const { VISION_MODEL, VISION_MODEL_CHAIN } = await loadAnalyzer({
      GEMINI_MODEL: " models/gemini-2.5-flash ",
    });

    expect(VISION_MODEL).toBe("gemini-2.5-flash");
    expect(VISION_MODEL_CHAIN).toEqual([
      "gemini-2.5-flash",
      "gemini-2.0-flash",
      "gemini-1.5-flash-8b",
  ...
  it("does not repeat the default model when it is configured explicitly", async () => {
    const { VISION_MODEL_CHAIN } = await loadAnalyzer({
  ...
  it("expands gemini-1.5-flash to the -latest alias", async () => {
    const { VISION_MODEL } = await loadAnalyzer({
      GEMINI_MODEL: "gemini-1.5-flash",
    });

    expect(VISION_MODEL).toBe("gemini-1.5-flash-latest");
  });

  it("accepts OPENAI_VISION_MODEL for compatibility", async () => {
    const { VISION_MODEL } = await loadAnalyzer({
      OPENAI_VISION_MODEL: "gemini-1.5-pro",
    });

    expect(VISION_MODEL).toBe("gemini-1.5-pro");
  });
});
...
describe("analyzeScreenshots", () => {
  ...
  it("retries the next model on 404 and keeps the request body", async () => {
    const { analyzeScreenshots } = await loadAnalyzer({
      GEMINI_API_BASE: "https://gem.test/v1beta",
    ...
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
      ...
      "https://gem.test/v1beta/models/gemini-2.0-flash:generateContent",
    ]);
  });

  it("surfaces a 404 of the last model in the chain and lists available models", async () => {
    const { analyzeScreenshots, VISION_MODEL_CHAIN, GEMINI_API_BASE } =
      await loadAnalyzer();
    fetchMock.mockImplementation(async (input) =>
      String(input).includes("/models?")
        ? geminiResponse({ models: [{ name: "models/gemini-real" }] })
        : geminiResponse({ error: { code: 404, message: "model gone" } }, 404),
    );

    await expect(analyzeScreenshots("/old.png", "/new.png")).rejects.toThrow(
      "Vision API вернул ошибку 404: model gone",
    );
    // Все модели цепочки плюс служебный GET со списком моделей.
    expect(fetchMock).toHaveBeenCalledTimes(VISION_MODEL_CHAIN.length + 1);
  ...
  it.each([
    ...
    "returns the fallback verdict for %s",
    async (_case: string, response: Response) => {
      const { analyzeScreenshots, UNPARSEABLE_ANALYSIS } = await loadAnalyzer();
