...
describe("analyzeScreenshots", () => {
  it("sends both screenshots as inline base64 PNG data", async () => {
    ...
    fetchMock.mockResolvedValue(geminiResponse(candidate(JSON.stringify(VERDICT))));
  ...
  it("surfaces a 404 of the last model in the chain", async () => {
    const { analyzeScreenshots, VISION_MODEL_CHAIN } = await loadAnalyzer();
    ...
    ).rejects.toThrow("Vision API вернул ошибку 404: model gone");
    expect(fetchMock).toHaveBeenCalledTimes(VISION_MODEL_CHAIN.length);
  });
